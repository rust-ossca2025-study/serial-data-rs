...
impl CustomerType {
    pub fn new() -> Self {
        CustomerType { name: "CustomerType".to_owned(), digit: 1, customer_type: None }
    }
}
...
impl GenSerialData for CustomerType {
    fn get_input_from_user(&mut self) {
        ...
        print!("{}-{:?}, ", usize::from(CustomerKind::Business), CustomerKind::Business);
        print!("{}-{:?}, ", usize::from(CustomerKind::Student), CustomerKind::Student);
        println!("{}-{:?}) ", usize::from(CustomerKind::Company), CustomerKind::Company);
